...
class DrawingClassifier:
    ...
    def init_gui(self):
        ...
        self.root = Tk()
        self.root.title(f"Drawing Classifier- {self.project_name}")
        ...
        class1_btn = Button(btn_frame, text=self.class1, command=lambda: self.save_image(1))
        class1_btn.grid(row=0, column=0, sticky=W+E)
        
        class2_btn = Button(btn_frame, text=self.class2, command=lambda: self.save_image(2))
        class2_btn.grid(row=0, column=1, sticky=W+E)
        
        class3_btn = Button(btn_frame, text=self.class3, command=lambda: self.save_image(3))
    ...
    def save_image(self, class_numb):
    ...
    def train_model(self):
        ...
        tkinter.messagebox.showinfo("Success", "Model trained successfully!", parent=self.root)
    ...
    def save_model(self):
        ...
        tkinter.messagebox.showinfo("Success", "Model saved successfully!", parent=self.root)
    
    def load_model(self):
        file_path = filedialog.askopenfilename()
        with open(file_path, 'rb') as f:
            self.clf = pickle.load(f)
        tkinter.messagebox.showinfo("Success", "Model loaded successfully!", parent=self.root)
    ...
    def predict(self):
        ...
        if prediction[0] == 1:
            tkinter.messagebox.showinfo("Prediction", f'The drawing might be a: {self.class1}', parent=self.root)
        elif prediction[0] == 2:
            tkinter.messagebox.showinfo("Prediction", f'The drawing might be a: {self.class2}', parent=self.root)
        elif prediction[0] == 3:
            tkinter.messagebox.showinfo("Prediction", f'The drawing might be a: {self.class3}', parent=self.root)
    
    def save_everything(self):
        data ={"c1": self.class1, "c2": self.class2, "c3": self.class3, "c1c": self.class1_counter, "c2c": self.class2_counter, "c3c": self.class3_counter, 'clf': self.clf, 'project_name': self.project_name}
        
        with open(f'{self.project_name}/{self.project_name}_data.pickle', 'wb') as f:
            pickle.dump(data, f)
        
        tkinter.messagebox.showinfo("Success", "Everything saved successfully!", parent=self.root)
    
    def mainloop(self):
        self.root.mainloop()
        
    def on_closing(self):
        answer = tkinter.messagebox.askquestion("Exit", "Do you want to save everything before exiting?", parent=self.root)
